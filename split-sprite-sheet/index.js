...
function reflectImage2Canvas(imageDom, dx, dy, imageDomWidth, imageDomHeight, itemDomWidth, itemDomHeight) {
  return new Promise((resolve) => {
    ...
    canvasDomContext.drawImage(imageDom, dx, dy, itemDomWidth, itemDomHeight, 0, 0, itemDomWidth, itemDomHeight);
...
function getDataURL(canvasDom) {
  return new Promise((resolve) => {
    const dataURL = canvasDom.toDataURL('image/png', 1.0);
    resolve(dataURL);
  });
}

function spliter(contentDom, itemDomWidth, itemDomHeight) {
  return new Promise(async (resolve) => {
    ...
    for (let y = 0; y < spliterationYCount; y++) {
      for (let x = 0; x < spliterationXCount; x++) {
        const canvasDom = await reflectImage2Canvas(
          imageDom,
          x * itemDomWidth,
          y * itemDomHeight,
          imageDomWidth,
        ...
        canvasDom.classList.add(`item-${c}`);
...
function crop(imageDom, itemDomWidth, itemDomHeight) {
  return new Promise((resolve) => {
    const canvasDom = document.createElement(`canvas`);
    const canvasDomContext = canvasDom.getContext('2d');

    canvasDom.width = itemDomWidth;
    canvasDom.height = itemDomHeight;
    canvasDomContext.drawImage(imageDom, 0, 0, itemDomWidth, itemDomHeight, 0, 0, itemDomWidth, itemDomHeight);
...
async function split() {
  ...
  const itemDomWidth = imageDomWidth / col;
  const itemDomHeight = imageDomHeight / row;

  const splitCount = await spliter(contentDom, itemDomWidth, itemDomHeight);

  const perPercent = 100 / splitCount;

  for (let index = 1; index <= splitCount; index++) {
    const canvasDom = document.querySelector(`.item-${index}`);
    if (canvasDom) {
      const dataURL = await getDataURL(canvasDom);
      const resultInfo = await loadImageDom(dataURL);
      const croppedCanvasDom = await crop(resultInfo.imageDom, itemDomWidth, itemDomHeight);
      const croppedDataURL = await getDataURL(croppedCanvasDom);
      const croppedResultInfo = await loadImageDom(croppedDataURL);
      document.querySelector(`.output`).appendChild(croppedResultInfo.imageDom);
